...
actor RoamDataHandler {
    ...
    private static func getForShared() -> Self {
        ...
        do {
            ...
            #endif
            let database = try RoamDatabase(
                databaseURL: databaseURL, lockURL: lockURL, legacyRootPath: rootPath)
            return self.init(
                database: database,
    ...
    private static func makeVolatileShared(
        ...
        do {
            ...
            let dataError = DataHandlerError.from(error: error)
            Log.backend.error(
                "Failed to open volatile Roam database; falling back to bare in-memory store: \(dataError, privacy: .public)"
            )
            DatabaseStatusMonitor.shared.setIssue(
                DatabaseStatusIssue(
                    error: dataError,
                    isVolatile: false,
                    operation: .open))
    ...
    func retryOpeningPersistentDatabase() async {
        ...
        do {
            ...
            Log.backend.warning(
                "Persistent Roam database retry failed: \(dataError, privacy: .public)")
        }
    }
    ...
    func requestDevice(id: String) -> Device? {
        ...
        do {
            ...
            Log.data.error(
                "Error loading device \(id, privacy: .public): \(error, privacy: .public)")
    ...
    func requestDeviceApps(deviceId: String) -> [AppLink] {
        ...
        do {
            ...
            Log.data.error(
                "Error loading device apps \(deviceId, privacy: .public): \(error, privacy: .public)"
            )
    ...
    func requestAllDevices(_ deviceIds: [String]) async -> [Device] {
        ...
        for deviceId in deviceIds {
            ...
            do {
                ...
                Log.data.error(
                    "Failed to load device \(deviceId, privacy: .public): \(error, privacy: .public)"
                )
            }
        }
    ...
    func addDevice(
        location: String, friendlyDeviceName: String, udn: String, serial: String,
        hidden: Bool = false
    ) async throws -> String {
        let device = Device(
    ...
    func setSelectedApp(deviceId: String, appId: String) throws {
        Log.data.info(
            "App selected \(appId, privacy: .public) for device \(deviceId, privacy: .public)")
    }
    ...
    func markMessagesViewed() async throws {
            ...
            do {
                ...
                if let lastSupportTyping = updates.presence.lastSupportTyping {
                    UserDefaults.standard.set(
                        lastSupportTyping.timeIntervalSince1970,
                        forKey: UserDefaultKeys.lastSupportTypingTime)
                }
        ...
        func refreshMessagesIfExpectingNewMessages() async -> Int {
            guard
                UserDefaults.standard.bool(forKey: UserDefaultKeys.hasSentFirstMessage)
                    || hasPendingMessagesToSend()
            else {
                return 0
            }
        ...
        func sendChatMessage(message: String, attachment: AttachmentUpload?) async throws {
            ...
            Log.backend.notice(
                "sendChatMessage started nonce=\(nonce, privacy: .public) contentBytes=\(message.utf8.count, privacy: .public) attachment=\(attachment?.filename ?? "--", privacy: .public)"
            )
            ...
            Log.backend.notice(
                "Saved pending message nonce=\(nonce, privacy: .public) pendingId=\(pendingMessage.id, privacy: .public)"
            )
            refreshMessageCache()
        ...
        private func sendPendingMessagesIfIdle(reason: String, force: Bool = false) async -> Int {
            ...
            if sentCount > 0 {
                Log.backend.notice(
                    "Finished pending message send reason=\(reason, privacy: .public) sentCount=\(sentCount, privacy: .public)"
                )
            }
            return sentCount
        }
...
extension RoamDataHandler {
    ...
    // MARK: - Device Management Extension
        ...
        @discardableResult
        func addOrReplaceDevice(location: String, id: String? = nil, serial: String? = nil)
            async throws -> String
        {
            ...
            do {
                ...
                if let device = requestDevice(id: info.udn) {
                    ...
                    Log.data.info(
                        "Found device for serial \(info.serial, privacy: .public), updating")
                    var updatedDevice = device
                ...
                Log.data.warning(
                    "Trying to add device but no preconnection info available \(location, privacy: .public). Error: \(error, privacy: .public)"
                )
                throw error
            }
            ...
            do {
                ...
                Task {
                        ...
                        do {
                            ...
                            Log.data.warning(
                                "Error refreshing device b/c no ECP Session: \(error, privacy: .public)"
                            )
                            return
                        }
                        defer {
                        ...
                        let refreshClient = ECPWebsocketRefreshClient(
                            id: device.id, client: ecpClient, location: device.location)
                    #endif
                ...
                Log.data.warning(
                    "Error adding device at \(location, privacy: .public), \(error, privacy: .public)"
                )
                throw error
            }
        }
        ...
        func sentToWatch(deviceId: String) async {
            do {
                ...
                Log.data.warning(
                    "Error marking device \(deviceId, privacy: .public) as sent to watch: \(error, privacy: .public)"
                )
            }
        }
        ...
        func resetWatchData() async {
            do {
                ...
                Log.data.warning(
                    "Error marking devices as not sent to watch: \(error, privacy: .public)")
            }
        }
    #endif
}
    ...
    extension RoamDataHandler {
        ...
        func refreshDevice(client: any RefreshClient) async {
            ...
            do {
                ...
                Log.data.error(
                    "Failed to refresh device because couldn't get id: \(error, privacy: .public)")
                return
            }
            ...
            if lastRefreshed.advanced(by: RoamDataHandler.minRescanInterval) > .now {
                Log.data.notice(
                    "Device refresh skipped for id \(deviceId, privacy: .public) b/c last refreshed \(lastRefreshed, privacy: .public)"
                )
                return
            } else {
                Log.data.notice(
                    "Refreshing device with id \(deviceId, privacy: .public) lastRefreshed \(lastRefreshed, privacy: .public)"
                )
            }
            ...
            do {
                ...
                Log.data.error(
                    "Failed to get device info \(deviceId, privacy: .public), \(error, privacy: .public)"
                )
                return
            }
            ...
            if let existingDevice = existingDevice {
                if deviceInfo.udn != existingDevice.udn {
                    Log.data.warning(
                        "Error: trying to refresh device with udn \(deviceInfo.udn, privacy: .public), but device already has udn \(existingDevice.udn, privacy: .public)"
                    )
                    return
                }
            ...
            let shouldSkipFullScan =
                (device.lastScannedAt?.timeIntervalSinceNow ?? -10000.0) > -RoamDataHandler
                .minRescanInterval && existingApps.allSatisfy({ $0.iconHash != nil })
            ...
            if let fetchedApps = fetchedApps {
                ...
                for app in afterUpdateApps
                where
                    (app.iconHash == nil
                    || (app.lastSyncAt ?? .distantPast).advanced(by: 3600 * 24) < .now)
                {
...
extension RoamDataHandler {
    ...
    private func loadTestData() async throws {
        ...
        try await clearData()
            ...
            Log.data.info(
                "Loaded test data: \(deviceCount) devices, \(totalApps) total apps, \(messageCount) messages"
            )
        #endif
    }
