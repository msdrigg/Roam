#!/usr/bin/env python3
"""One-off: re-capture Apple Vision Pro ar-SA after RTL fix."""
...
def main():
    ...
    mm = sync_metadata.MetadataManager(
        workspace_path=".",
        screenshot_update_platforms=[sync_metadata.Platform.visionOS],
    )

    import types
    def _filtered(self, lid):
        device = ScreenshotSource(device=device_name, size="3840_2160")
        path = self._get_device_screenshots(device.device, lid)
        if path:
            return {sync_metadata.Platform.visionOS: [ScreenshotExport(size=device.size, path=path)]}
        return {}
