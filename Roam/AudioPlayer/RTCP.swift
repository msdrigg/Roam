...
enum RtcpPacket {
    ...
    init?(data: Data) {
        ...
        if version != 2 {
            Log.headphones.warning("Bad rtcp packet recevied with version = \(version, privacy: .public). Expected version = 2")
            return nil
        }
        ...
        case 201:
            guard let packet = RtcpReceiverReport(data: packetData) else {
                Log.headphones.warning("Bad receiver report recevied with data \(packetData, privacy: .public)")
                return nil
            }
            self = Self.receiverReport(packet)
        case 203:
            guard let packet = RtcpBye(data: packetData) else {
                Log.headphones.warning("Bad bye packet recevied with data \(packetData, privacy: .public)")
                return nil
            }
            self = Self.bye(packet)
        case 204:
            guard let packet = RtcpAppSpecific(subtypeData: subtypeData, packet: packetData) else {
                Log.headphones.warning("Bad app packet recevied with data \(packetData, privacy: .public)")
                return nil
            }
...
enum RtcpAppSpecific {
    ...
    init?(subtypeData: UInt8, packet: Data) {
        ...
        guard let name = String(data: nameData, encoding: .utf8) else {
            Log.headphones.warning("Bad data for name in app data \(nameData, privacy: .public)")
            return nil
        }
...
struct RtcpVdly {
    ...
    init?(data: Data) {
        guard let delay = UInt32(bigEndian: data) else {
            Log.headphones.warning("Bad data for vdly delay in app data \(data, privacy: .public)")
            return nil
        }
...
struct RtcpCver {
    ...
    init?(data: Data) {
        guard let clientVersion = UInt32(bigEndian: data) else {
            Log.headphones.warning("Bad data for client version in app data \(data, privacy: .public)")
            return nil
        }
        self.clientVersion = clientVersion
    }
