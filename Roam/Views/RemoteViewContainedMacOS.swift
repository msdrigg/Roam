...
struct RemoteViewContained: View {
    ...
    @AppStorage(UserDefaultKeys.localNetworkPermissionGranted) private var networkPermissionGranted = false
    @AppStorage(UserDefaultKeys.shouldScanIPRangeAutomatically) private var shouldScanIPRangeAutomatically = true
    ...
    private var isLoadingDevices: Bool {
        if devicesLoader.isLoading || primaryDeviceLoader.isLoading || devicesLoader.devices == nil {
            return true
        }
    ...
    private var configuredContent: some View {
        content
            ...
            .task(
                id: "\(scanningActor != nil && selectedDevice == nil && scanSSDP)-\(networkMonitor.networkConnection)"
            ) {
    ...
    private var remoteDetail: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                if unreadMessages > 0 {
                    NotificationBanner(
                        message: String(localized: "The developer chatted you back",
                                        comment: "Notification indicator that there was is message response waiting to be read"),
                        onClick: {
    ...
    private func ensureSelectedDevice() async {
        ...
        guard selectedDevice == nil || !deviceIds.contains(selectedDevice?.id ?? "") else { return }

        do {
            try await RoamDataHandler.shared.makePrimaryDevice(id: firstDeviceId)
        } catch {
            Log.userInteraction.error("Error setting initial selected device \(error, privacy: .public)")
        }
    }
    ...
    private func moveDevices(fromOffsets: IndexSet, toOffset: Int) {
        Task {
            do {
                try await RoamDataHandler.shared.reorderDevices(fromOffsets: fromOffsets, toOffset: toOffset)
            } catch {
...
private struct MacDeviceSidebarItem: View {
    ...
    init(id: String) {
        self.id = id
        deviceLoader = DeviceLoader(deviceId: id, dataHandler: .shared)
    }
    ...
    var body: some View {
        Label {
            ...
            Image(systemName: (deviceLoader.device?.isOnline() ?? false) ? "tv.fill" : "tv")
                .foregroundStyle((deviceLoader.device?.isOnline() ?? false) ? Color.accentColor : Color.secondary)
        }
    }
}
...
private struct MacDeviceToolbarMenuItem: View {
    ...
    var body: some View {
        Button(action: action) {
            Label(deviceLoader.device?.name ?? "Loading...", systemImage: selectedDeviceId == id ? "checkmark" : "tv")
        }
    }
}
