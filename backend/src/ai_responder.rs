...
Roam's keyboard shortcuts (the hardware-keyboard keys mapped to remote actions) are customizable in the app at Settings -> Keyboard shortcuts on macOS, iOS, iPadOS, and visionOS (not watchOS). This is separate from on-screen text entry. When a user wants to change, reset, or clear a keyboard shortcut, point them there.
